...
# `python scripts/transfer_pokemon.py` puts scripts/ on the path, not the checkout root
# that holds `sources`.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from sources import fleet_entrypoint, pokemon_fleet

def main(arguments=None):
    return fleet_entrypoint.run_operation('delete', 'scripts/transfer_pokemon.py', arguments)
